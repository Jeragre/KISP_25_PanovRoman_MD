...
## Учебник: Создайте приложение с агентом ИИ

В этом уроке вы создадите полное приложение для Android, iOS и Интернета без написания кода самостоятельно. Вместо этого вы направите агент кодирования ИИ, такой как Клод, Кодекс или Курсор, и проверите результаты в прямом эфире на своем собственном телефоне после каждого шага.

Этот учебник предназначен для buildersстроителей: людей, у которых есть идея для приложения, но нет опыта программирования. Если вы предпочитаете писать код самостоятельно, следуйте учебнику Экспо. Он создает одно и то же приложение, один фрагмент кода за раз.

### Что вы построите

Вы создадите StickerSmash, приложение, которое позволяет вам выбрать фотографию, разместить на нем наклейку смайликов, перемещать и изменять размер наклейки пальцами и сохранять результат в библиотеке фотографий:

### Как работает этот учебник

Каждая глава следует одной и той же петли:    
1. Подсказка: вставьте предлагаемую подсказку в свой агент ИИ.
2. BuildАгент записывает код и обновление приложения на вашем телефоне в течение нескольких секунд.    
3. Проверьте: проверьте, что приложение делает то, что вы просили, прямо на вашем телефоне.    
4. Повторный: если что-то выключено, скажите агенту, что вы видите, и позвольте ему решить проблему.

Вы являетесь владельцем продукта и тестировщиком. Агентом является программист.
Агенты ИИ не дают одинаковых результатов каждый раз, поэтому ваше приложение не будет соответствовать нашему пикселю скриншотов для пикселей. Это ожидаемо. Подсказки описывают желаемый результат, а скриншоты показывают примерно то, что вы должны видеть. Не стесняйтесь редактировать подсказки, чтобы соответствовать вашему собственному вкусу: выберите разные цвета, измените формулировку на кнопках, сделайте ее своей.

### Что вам понадобится

Компьютер под управлением macOS, Windows или Linux.
Телефон Android или iOS.
Около часа. Первая глава рассказывает вам об установке каждого инструмента с нуля, поэтому вам не нужно ничего настраивать заранее.
...
В каталоге проекта запустите следующую команду для запуска сервера разработки с терминала: npx expo start
...
3. Переместите существующие файлы index.tsx и about.tsx в каталог (вкладки). Структура каталога src/app будет выглядеть так:
```
src 
app  
_layout.tsx
...
Чтобы установить @expo/vector-icons библиотека, остановите сервер разработки, нажав Ctrl + C в терминале, затем запустить следующую команду: npx expo install @expo/vector-icons
...
## Создайте экран
В этом уроке узнайте, как использовать такие компоненты, как React Native Pressable и Expo Image, чтобы построить экран.
...
```
@ Символ – это обычай Путь псевдоним для импорта пользовательских компонентов и других модулей вместо относительных путей. Expo CLI автоматически настраивает его в tsconfig.json.
...
В этом уроке узнайте, как использовать Expo Image Picker.
...
Мы будем использовать expo-image-pickerБиблиотека от Экспо SDK. expo-image-picker обеспечивает доступ к пользовательскому интерфейсу системы для выбора изображений и видео из библиотеки телефона.
### Установить expo-image-Picker
Чтобы установить expo-image-picker библиотека, остановите сервер разработки, нажав Ctrl + C в терминале, затем запустить следующую команду: npx expo install expo-image-picker
The npx expo install команда установит библиотеку и добавит ее в зависимости проекта в package.json.

>Совет: Каждый раз, когда мы устанавливаем новую библиотеку в проект, остановите сервер разработки нажатием Ctrl + C в терминале, а затем запустить команду установки. После завершения установки запустите сервер разработки, запустив npx expo start.
...
```
pickImageAsync() Функции вызывают ImagePicker.launchImageLibraryAsync() А затем обрабатывает результат. The launchImageLibraryAsync() Способ возвращает объект, содержащий информацию о выбранном изображении.
...
reslult Объект обеспечивает assets массив, который содержит uri Выбранное изображение. Давайте возьмем это значение из сборщика изображений и используем его, чтобы показать выбранное изображение в приложении.
...
## Создать модаль 
В этом уроке узнайте, как создать модаль React Native для выбора изображения.
...
```
### Cоздать модаль сборщика смайликов
...
```
## Добавить жесты
В этом уроке узнайте, как реализовать жесты из React Native Gesture Handler и Reanimated библиотек.
...
```
## Сделать скриншот
В этом уроке узнайте, как сделать снимок экрана с помощью сторонней библиотеки и библиотеки Expo Media.
...
Установить ```react-native-view-shot``` и ```expo-media-library```, выполните следующие команды:```npx expo install react-native-view-shot expo-media-library```
...
В этом уроке узнайте, как справляться с различиями платформы между нативными и веб-сайтами при создании универсального приложения.
...
Остановите сервер разработки и выполните следующую команду для установки библиотеки: ```npm install dom-to-image```
...
```
Исправление dom-to-imageОшибка модуля TypeScript

Нам нужно добавить определение типа после импорта domtoimage библиотека, так как мы используем TypeScript. Мы можем сделать это, создав файл Тип.д.т в корне нашего каталога проекта и добавление заявления декларации: ```declare module 'dom-to-image';```
## Настройка панели состояния, экрана брызг и значка приложения
В этом уроке изучите основы настройки панели состояния, значка приложения и экрана брызг.
...
Вы можете увидеть иконку в различных местах в Экспо Го. Вот пример значка приложения, отображаемого в меню разработчика Expo Go:
### Всплеск экрана
...
```
Однако для тестирования экрана брызг мы не можем использовать Expo Go или сборку разработки. Чтобы протестировать его, нам нужно создать предварительный просмотр или производственную сборку нашего приложения. Мы рекомендуем пройти следующие ресурсы, чтобы узнать больше о конфигурации экрана брызг и о том, как ее протестировать:

    Создайте руководство по иконке брызг экрана, чтобы узнать, как настроена значок брызг экрана.
    Чтобы узнать, как создать сборку предварительного просмотра, см. Руководство по внутреннему распространению в EAS Tutorial или для создания производственных сборок см. руководства для Android и iOS.
## Учебные ресурсы
Исследуйте подготовленный список ресурсов, чтобы узнать о Expo и React Native.
